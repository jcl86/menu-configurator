...
namespace MenuConfigurator.Domain
{
    public static class Ensure
    {
        ...
        {
            That<ArgumentNullException>(value != null, message);
        }
        ...
        {
            That<ArgumentNullException>(!string.IsNullOrEmpty(value), message);
        }
        ...
        public static void Equal<T, TException>(T left, T right, string message = "Values must be equal") where TException : Exception
        {
            That<TException>(left != null && right != null && left.Equals(right), message);
        }
        ...
        /// <remarks>Null values will cause an exception to be thrown</remarks>
        public static void NotEqual<T, TException>(T left, T right, string message = "Values must not be equal") where TException : Exception
        {
            That<TException>(left != null && right != null && !left.Equals(right), message);
        }
        ...
        /// </exception>
        public static void Contains<T, TException>(IEnumerable<T> collection, Func<T, bool> predicate, string message = "") where TException : Exception
        {
            That<TException>(collection != null && collection.Any(predicate), message);
        }
        ...
        /// </exception>
        public static void Items<T, TException>(IEnumerable<T> collection, Func<T, bool> predicate, string message = "") where TException : Exception
        {
            That<TException>(collection != null && !collection.Any(x => !predicate(x)), message);
        }
        ...
        public static class Argument
        {
            ...
            /// <summary>
            /// Ensures given string is null or its length is not above limit
            /// </summary>
            /// <param name="value">Value to test for null</param>
            /// <param name="paramName">Name of the parameter in the method</param>
            /// <exception cref="ArgumentNullException">
            ///     Thrown if <paramref cref="value" /> is null
            /// </exception>
            public static void FitsToLength(string value, int limit, string paramName = "")
            {
                if (value != null)
                {
                    That<ArgumentNullException>(value.Length <= limit, paramName);
                }
            }
        }
    }
